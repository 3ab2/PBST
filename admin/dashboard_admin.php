...
<div class="row mb-4">
    ...
    <div class="col-lg-3 col-md-6 col-12 mb-3">
        <div class="card text-center">
            <div class="card-body" style="min-height: 120px;">
                <h5 class="card-title"><?php echo htmlspecialchars($translations['consultations']); ?></h5>
    ...
    <div class="col-lg-3 col-md-6 col-12 mb-3">
        <div class="card text-center">
            <div class="card-body" style="min-height: 120px;">
                <h5 class="card-title"><?php echo htmlspecialchars($translations['users']); ?></h5>
                <p class="card-text fs-3" id="cardUsersCount">0</p>
            </div>
        </div>
    </div>
    <div class="col-lg-3 col-md-6 col-12 mb-3">
...
<div class="row">
    ...
    <div class="col-md-6 mb-4">
        <div class="card">
            <div class="card-header"><?php echo htmlspecialchars($translations['manage_consultations'] ?? 'إدارة الاستشارات'); ?></div>
            <div class="card-body" style="height: 300px;">
                <canvas id="consultationsChart"></canvas>
            </div>
            <div class="card-footer">
                <a href="manage_consultations.php" class="btn btn-primary btn-sm"><?php echo htmlspecialchars($translations['manage'] ?? 'إدارة'); ?></a>
            </div>
        </div>
    </div>
    <div class="col-md-6 mb-4">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateCards() {
        ...
        // Consultations
        fetch('chart_data.php?type=consultations')
            .then(response => response.json())
            .then(data => {
                const total = data.reduce((sum, item) => sum + parseInt(item.count), 0);
        ...
        // Users
        fetch('chart_data.php?type=users')
            .then(response => response.json())
            .then(data => {
                const total = data.reduce((sum, item) => sum + parseInt(item.count), 0);
                document.getElementById('cardUsersCount').textContent = total;
            });
    ...
    function updateCharts() {
        ...
        // Consultations - Number of consultations per month
        fetch('chart_data.php?type=consultations')
            .then(response => response.json())
            .then(data => {
